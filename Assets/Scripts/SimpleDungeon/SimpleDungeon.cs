...
public class SimpleDungeon : MonoBehaviour
{
    ...
    public int minSize = 4;                         // 방 최소 크기 (타일 단위, 가로/새로 동일)
    public int maxSize = 8;                         // 방 최대 크기 (타일 단위, 가로/새로 동일)
    ...
    void CreateRooms()
    {
        ...
        for(int i = 0; i < treasureCount && normalRoom.Count > 0; i++)      // 무작위 일반방을 보물방으로 바꾼다.
        {
            int idx = Random.Range(0, normalRoom.Count);
            normalRoom[idx].type = RoomType.Treasure;
            normalRoom.RemoveAt(idx);
        }
    }    
    ...
    void CreateCorridor(Vector2Int start, Vector2Int end)
    {
        ...
        // x 축 정렬 : start.x -> end.x 로 한칸 씩 이동하며 바닥 타일 추가
        ...
        // y 축 정렬 : x가 같아진 뒤 start.y -> end.y 로 한 칸씩 이동
    ...
    void Render()
    {
        ...
        foreach(var pos in floors)
        {
            ...
            cube.transform.localScale = new Vector3(1f, 0.1f, 1f);                      // 얇은 바닥 설치
        ...
        {
            GameObject cube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cube.transform.position = new Vector3(pos.x, 0.5f, pos.y);                  // y = 0.5 평면에 배치
            cube.transform.SetParent(transform);                                        // 부모 지정
    ...
    {
        float halfSize = room.size / 2f - 1;            // -1 테두리
    ...
    void CreateTreasure(Vector3 position)
    {
        ...
        treasure.AddComponent<TreasureItem>();                              // 보물 줍기 추가
    }
