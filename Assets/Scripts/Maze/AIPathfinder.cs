...
public class AIPathfinder : MonoBehaviour                               // BFS를 사용해서 AI가 자동으로 목표까지 이동
{
    ...
    // 이동 가능한 이웃 찾기
    List<MazeCell> GetAccessivleNeighbors(MazeCell cell)
    {
    ...
    List<MazeCell> FindPathBFS(MazeCell start, MazeCell end)
    {
        ...
        while(queue.Count > 0 )
        {
            ...
            List<MazeCell> neighbors = GetAccessivleNeighbors(current);
    ...
    void MoveAlongPath()
    {
        ...
        MazeCell targetrCell = currentPath[pathIndex];
        targetPosition = new Vector3(
            targetrCell.x * MazeGenerator.Instance.cellSize,
            transform.position.y,
            targetrCell.z * MazeGenerator.Instance.cellSize
            );
