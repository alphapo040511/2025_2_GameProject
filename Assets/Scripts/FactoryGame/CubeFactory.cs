...
public class CubeFactory : MonoBehaviour
{
    ...
    // 자료 구조들
    private Queue<GameObject> materialQueue = new Queue<GameObject>();                      // 입고 완료 큐
    ...
    private List<WorkRequest> requestList = new List<WorkRequest>();                        // 요청서 스택
    private Dictionary<ProductType, int> products = new Dictionary<ProductType, int>();     // 완재품 딕셔너리
    ...
    void AddMaterial()
    {
        ...
        ResourceCube cubeCompoenent = newCube.AddComponent<ResourceCube>();
        cubeCompoenent.Initialize(randomType);
    ...
    void OnGUI()
    {
        ...
        for(int i = 0; i < requestList.Count && i < 3; i++)
        {
            ...
            GUI.Label(new Rect(10, 190 + i * 20, 300, 20),
                $"[{i} 의자 {request.quantity} -> {request.reward} 원");
        }
