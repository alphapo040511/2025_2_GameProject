...
public class CityGenerator : MonoBehaviour
{
    ...
    [Range(0f, 1f)] public float plazeChance = 0.05f;
            ...
            for( int z = 0;z < cityHeight; z++)
            {
                if(!IsRoad(x, z))                                                       // 추후 랜덤으로 타워, 광장, 공원을 추가
                {
                    float rand = Random.value;
                    {
                        ...
                        else if(rand < towerChance + plazeChance)
                        {
                            CreatePlaza(x, z);
                        }
                        else if(rand < towerChance + plazeChance + parkChance)
                        {
                            CreatePark(x, z);
                        }
                        else
    ...
    {
        float disFormCenter = Vector2.Distance(new Vector2(x, z), new Vector2(cityWidth / 2f, cityHeight / 2f));

        if(disFormCenter < cityWidth * 0.2f)
        {
            return new Color(0.6f, 0.6f, 0.7f);
        }
        else if (disFormCenter < cityWidth * 0.4f)
        {
