...
public class VoxelChunk : MonoBehaviour
{
    ...
    void SetupMesh()
    {
        ...
        chunkMesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32;               // 128 넘어가도 잘 동작하기 위해 (기본 = UInt16)
    }
    ...
    int GetTerrainHeight(int worldX, int worldZ)
    {
        ...
        for(int i = 0; i < octaves; i++)
        {
            ...
            noiseHeight += perlinValue * amplitude;                         // 진폭을 이용해서 누적
    ...
    // 한 명 추가
    void AddFace(int x, int y, int z, Vector3 direction, Color color, List<Vector3> vertices, List<int> triangles, List<Color> colors)
    {
